...
const normalizeEmotionData = (emotionData: EmotionData | null | undefined): NormalizedEmotionData => {
  ...
  let data = emotionData || defaultEmotionData;
  
  // Handle case when emotion_data is a string
  if (typeof data === 'string') {
    try {
      data = JSON.parse(data);
    } catch (e) {
      console.warn('Failed to parse emotion_data string:', e);
  ...
  if (!data || typeof data !== 'object') {
    console.warn('Invalid emotion_data format:', data);
    data = defaultEmotionData;
  }
  
  // Check if we have a direct emotion probability object
  // (like the one generated by our scripts with RAF-DB categories)
  if (
    // Safe check for property existence to avoid "in" operator issues
    Object.prototype.hasOwnProperty.call(data, 'neutral') && 
    Object.prototype.hasOwnProperty.call(data, 'happiness') && 
    !Object.prototype.hasOwnProperty.call(data, 'probabilities')
  ) {
    const normalized: EmotionPrediction = {
      happiness: Number(data.happiness) || 0,
    ...
    // Calculate the dominant emotion and confidence
    const dominantEntry = Object.entries(normalized).reduce(
      (a, b) => (a[1] > b[1] ? a : b),
      ['neutral', 0]
    );
    ...
    return {
      ...
      probabilities: {
        neutral: Number(data.neutral) || 0,
        happy: Number(data.happiness) || 0,
        sad: Number(data.sadness) || 0,
        angry: Number(data.anger) || 0,
        surprised: Number(data.surprise) || 0,
        fearful: Number(data.fear) || 0,
        disgusted: Number(data.disgust) || 0
      },
