...
def parse_args():
    parser = argparse.ArgumentParser(description='Train the server model for facial recognition')
    parser.add_argument('--data-dir', type=str, default='data/partitioned/server',
                        help='Directory containing the server training data')
...
def main():
    ...
    EPOCHS = args.epochs
    MODEL_PATH = os.path.join(args.model_dir, 'server_model.h5')
    INIT_LR = args.lr
    ...
    # Load dataset
    try:
        print(f'Loading server dataset from {args.data_dir}...')
        ...
        print(f'Training server model with {num_classes} classes')
        ...
        print(f"Classes in server partition: {sorted(list(train_generator.class_indices.keys()))[:5]}...")
        ...
        model.summary()
        print("Phase 1: Training only the classifier head, base model frozen")

        # Set up callbacks
        callbacks_phase1 = [
            ModelCheckpoint(os.path.join(args.model_dir, 'server_model_phase1.h5'), 
        ...
        print(f'Phase 2: Fine-tuning with {sum(1 for layer in model.layers if layer.trainable)} trainable layers')
        ...
        import json
        with open(os.path.join(args.model_dir, 'server_class_indices.json'), 'w') as f:
        ...
        if 'history' in locals() and len(history.history.get('accuracy', [])) > 0:
            ...
            print(f'Final server model training accuracy: {final_accuracy:.4f}')
            print(f'Final server model validation accuracy: {final_val_accuracy:.4f}')
            print(f'Best validation accuracy achieved: {best_val_accuracy:.4f}')
        else:
            print('Training was interrupted, final metrics not available')

    except Exception as e:
        print(f'Error training server model: {str(e)}')
        import traceback
        traceback.print_exc()
        print('Skipping server model training')
